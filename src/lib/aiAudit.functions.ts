...
ИЕРАРХИЯ ИСТОЧНИКОВ (используй СТРОГО в этом порядке приоритета):
1. ОФИЦИАЛЬНЫЙ САЙТ АРХИВА (archival-services.gov.ge/saeklesio/ и связанные external_sources URL карточки) — ПЕРВИЧНЫЙ источник истины. Структура сайта: СПРАВА — вкладки УЕЗДОВ (данные ДО 1871 года), СЛЕВА — вкладки РАЙОНОВ (данные ПОСЛЕ 1871 года). Если карточка относится к периоду ≤1870 — ищи селение/церковь в правой колонке (уезд); если ≥1871 — в левой (район). При наличии URL архива в external_sources считай его авторитетным; цитируй его в sources.
2. ФРАГМЕНТЫ PDF метрических книг (если приложены) — вторичный источник, дополняет сайт. Используется когда сайт не подтверждает или не упомянут.
3. КАТАЛОГ НИАГ из контекста — третичный fallback (агрегированная выборка, может быть неполной).

Если ни один из трёх источников не подтверждает данные карточки — пиши confidence < 0.3 и оставляй *_ok=true. Никогда не выдумывай факты. Если уезд относится к общим регионам (Имеретия, Гурия, Абхазия, Мегрелия, Сванетия, Кахетия, Картли) и подробного уезда нет — не предлагай уточнений по уезду.

ОСОБЫЕ ПРАВИЛА ПО ГОДАМ (КРИТИЧНО):
1. Каталог НИАГ Ф.489 оп.6 и PDF покрывают ТОЛЬКО 1819–1870. Для лет ≥1871 опирайся ИСКЛЮЧИТЕЛЬНО на сайт архива (левая колонка районов). Если карточка содержит данные после 1870 года (напр. 1846–1916), и сайт архива их подтверждает — years_ok=true. ОТСУТСТВИЕ поздних лет в каталоге/PDF НЕ ошибка.
2. НИКОГДА не предлагай сокращать диапазон. Если источники подтверждают диапазон карточки — years_ok=true.
3. years_correction разрешён ТОЛЬКО если авторитетный источник (сайт > PDF > каталог) содержит даты ВНЕ карточного диапазона (требуется расширение startYear↓ или endYear↑). yearsRaw — объект {ru,en,ka} с одинаковой строкой "YYYY-YYYY" во всех языках. Обязательно укажи новые startYear ≤ endYear.
4. Не ломай формат: yearsRaw всегда трёхъязычный объект, startYear/endYear — целые числа.

Возвращай результат строго через инструмент propose_corrections. В поле sources перечисляй ИСТОЧНИКИ в порядке использования (URL сайта → PDF страница → цитата каталога).`;
