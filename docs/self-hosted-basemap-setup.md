...
1. У вас должен быть домен в Cloudflare (DNS управляется ими). Если домен
   `datatells.info` там, прекрасно — иначе сначала перенесите DNS.
2. В bucket → **Settings** → **Custom Domains** → **Connect Domain**.
3. Введите, например, `tiles.datatells.info`.
4. Cloudflare сам создаст CNAME запись и выпустит SSL — займёт пару минут.
5. Это будет ваш `VITE_BASEMAP_BASE_URL=https://tiles.datatells.info`.
